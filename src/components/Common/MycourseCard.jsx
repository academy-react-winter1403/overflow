...
import detail from '../../assets/userpanel/icons8-eye-24.png'

function MycourseCard({ item, index, handleNavigation, className = "" }) {
      ...
      <Link to={`/AllCourses/Courses/${item.courseId}`}>
        ...
        >
          ...
          <div className="flex w-9/10 flex-row-reverse text-right ">
            <h3 className="font-iransans max-md:text-sm  w-4/10 h-full truncate  pt-5 text-center text-xl font-bold text-gray-600  max-sm:h-10 max-sm:pr-3 max-sm:text-xs dark:text-white">
              {item.courseTitle}
            </h3>

            <div className="max-xl:hidden font-iransans font-semibol w-4/10  pt-5 text-xl text-center max-xl:w-6/10 max-sm:hidden">
              {item.fullName}
            </div>

            <div className="font-iransans text-center dark:text-white text-2xl pt-5 w-4/10 max-lg:hidden ">
              {new Date(item.lastUpdate).toLocaleTimeString("fa-IR")}
            </div>
            <div className="text-deep-blue flex w-4/10 max-md:w-3/10  truncate max-md:text-sm text-xl font-bold justify-center pt-5 dark:text-white ">
              ...
              <svg
                ...
                viewBox="0 0 24 24"
                fill="currentColor"
                className="h-10 w-10 dark:text-white max-md:hidden"
              >
            ...
            <button className=" w-2/10 h-5/10 m-auto justify-center flex flex-row ">
                <img src={pay} className="w-1/3 max-md:w-2/3 "/>
            </button>
          </div>
        </div>
      </Link>
